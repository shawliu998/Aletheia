...
function auditV15ClassificationUpgradeAndRestart() {
  ...
  try {
    assert.equal(classificationRequiredRestart.migration?.currentVersion, 26);
  ...
  try {
    assert.equal(classifiedRestart.migration?.currentVersion, 26);
    assert.deepEqual(
      object(
        classifiedRestart
          .prepare(
  ...
  try {
    assert.equal(currentBinaryRecovery.migration?.currentVersion, 26);
    ...
    JSON.stringify(
      {
        ...
        current_version: 26,
        checks: [
          "clean SQLite v26 install with additive inference-policy, Legal Provider, Assistant action, Draft metadata, templates, research, Tabular handoff, contract-review, task-event, and custom-extraction slices",
