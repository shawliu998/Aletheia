...
function auditV15ClassificationUpgradeAndRestart() {
  ...
  try {
    assert.equal(classificationRequiredRestart.migration?.currentVersion, 25);
  ...
  try {
    assert.equal(classifiedRestart.migration?.currentVersion, 25);
    assert.deepEqual(
      object(
        classifiedRestart
          .prepare(
  ...
  try {
    assert.equal(currentBinaryRecovery.migration?.currentVersion, 25);
    ...
    JSON.stringify(
      {
        ...
        current_version: 25,
        checks: [
          "clean SQLite v25 install with additive inference-policy, Legal Provider, Assistant action, Draft metadata, templates, research, Tabular handoff, contract-review, and task-event slices",
