...
function auditV15ClassificationUpgradeAndRestart() {
  ...
  try {
    assert.equal(classificationRequiredRestart.migration?.currentVersion, 18);
  ...
  try {
    assert.equal(classifiedRestart.migration?.currentVersion, 18);
    assert.deepEqual(
      object(
        classifiedRestart
          .prepare(
  ...
  try {
    assert.equal(currentBinaryRecovery.migration?.currentVersion, 18);
    ...
    JSON.stringify(
      {
        ...
        current_version: 18,
        checks: [
          "clean SQLite v18 install with additive inference-policy and Legal Provider slices",
