...
class FakeTabular {
  ...
  get(id: string) {
    if (!this.detail || this.detail.review.id !== id)
  ...
  runReview(id: string) {
    ...
    for (const [index, cell] of detail.cells.entries()) {
      ...
      cell.attempt = 1;
      cell.status = "queued";
      const versionId = DOCS.find(
        ([documentId]) => documentId === cell.documentId,
      )![1];
...
async function executeRun(
  module: WorkspaceAssistantContractReviewToolModule,
...
async function main() {
  ...
  const noReview = harness("timeout", 0);
  const noReviewContext = { ...context(), documents: [] };
  await noReview.module.registeredTools(noReviewContext);
  assert.equal(
    await noReview.module.settleLifecycle({
      phase: "before_final",
      context: noReviewContext,
      signal: new AbortController().signal,
    }),
    null,
    "final guard must be a no-op when the immutable snapshot could not own a review",
  );
  ...
  assert.deepEqual(
    ...
    ["tabular_review_created", "draft_created"],
  );
  const replay = await executeRun(success.module);
  assert.equal(
    success.tabular.creates,
    1,
    "deterministic replay must not duplicate review",
  );
  assert.equal(
    success.memos(),
    2,
    "memo handoff is invoked idempotently by its v23 boundary",
  );
  assert.deepEqual(
    replay.events,
    [],
    "same-attempt replay must not duplicate durable UI events",
  );
  ...
  const resumed = await timeout.module.execute({
    ...
    call: {
      ...
      input: { review_id: pendingPayload.review.review_id },
    },
    signal: new AbortController().signal,
  });
  assert.equal(JSON.parse(resumed.content).memo.draft_id, DRAFT);

  const lifecycle = harness("timeout", 0);
  await lifecycle.module.registeredTools(context());
  const lifecyclePending = await executeRun(lifecycle.module);
  assert.equal(JSON.parse(lifecyclePending.content).asynchronous, true);
  assert.deepEqual(
    lifecyclePending.events?.map((event) => event.type),
    ["tabular_review_created"],
  );
  lifecycle.tabular.mode = "success";
  const lifecycleSettled = await lifecycle.module.settleLifecycle({
  ...
  assert.ok(lifecycleSettled?.replacementContent);
  const settledPayload = JSON.parse(lifecycleSettled.replacementContent);
  assert.equal(settledPayload.asynchronous, false);
  assert.equal(settledPayload.review.status, "complete");
  assert.equal(settledPayload.memo.draft_id, DRAFT);
  assert.deepEqual(
    lifecycleSettled.events?.map((event) => event.type),
    ["draft_created"],
  );
  assert.equal(lifecycle.memos(), 1);

  const recovery = harness("timeout", 0);
  await recovery.module.registeredTools(context());
  const recoveryPending = await executeRun(recovery.module);
  assert.equal(JSON.parse(recoveryPending.content).asynchronous, true);
  recovery.tabular.detail.review.status = "complete";
  for (const cell of recovery.tabular.detail.cells) cell.status = "complete";
  const restarted = new WorkspaceAssistantContractReviewToolModule(
    recovery.database as any,
    () => recovery.tabular as any,
    recovery.workflows,
    recovery.actions as any,
    recovery.createMemo,
    () => true,
    {
      maxWaitMs: 0,
      initialPollMs: 1,
      maxPollMs: 1,
      delay: async () => recovery.tabular.settle(),
    },
  );
  ...
  assert.equal(
    recovery.memos(),
    1,
    "final guard must create the recovered memo exactly once",
  );

  const lifecycleCancelled = harness("timeout", 0);
  await lifecycleCancelled.module.registeredTools(context());
  const lifecycleController = new AbortController();
  const lifecycleCancelledPending = await executeRun(
    lifecycleCancelled.module,
    lifecycleController.signal,
  );
  lifecycleController.abort();
  await assert.rejects(
    lifecycleCancelled.module.settleLifecycle({
      phase: "after_execution",
      context: context(),
      call: {
        id: "call-run",
      ...
      result: lifecycleCancelledPending,
      signal: lifecycleController.signal,
    }),
    { name: "AbortError" },
  );
  assert.equal(lifecycleCancelled.tabular.cancels, 1);
  assert.equal(lifecycleCancelled.memos(), 0);
  ...
  controller.abort();
  await assert.rejects(() => executeRun(cancelled.module, controller.signal), {
    name: "AbortError",
  });
  assert.equal(
    cancelled.tabular.cancels,
    1,
    "pre-aborted signal must cascade after deterministic creation",
  );
